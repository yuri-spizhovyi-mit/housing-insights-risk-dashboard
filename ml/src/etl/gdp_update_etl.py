...
def load_gdp_zip(path="data/raw/gdp_36100434.zip") -> pd.DataFrame:
    ...
    df = df[
        ...
        & (
            df["North American Industry Classification System (NAICS)"].str.contains(
                "All industries"
            )
        )
    ].copy()
    ...
    print(
        f"[INFO] Filtered {len(df)} monthly GDP points (All industries, real chained 2017 dollars)"
    )
    return df
...
def transform_gdp(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(
        f"[INFO] Computed GDP YoY from {df['date'].min().date()} → {df['date'].max().date()}"
    )
    return df
