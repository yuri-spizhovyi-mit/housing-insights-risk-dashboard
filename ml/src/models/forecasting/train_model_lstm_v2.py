"""
train_model_lstm_v2.py
----------------------------------------------------------
Refined production-ready LSTM model training script.
Forecasts both 'price' (hpi_benchmark) and 'rent' (rent_avg_city) per city.
Includes EarlyStopping, Dropout, and model weight saving.
...
def train_lstm_dual_target(df: pd.DataFrame):
    ...
    os.makedirs("ml/models/lstm_v2", exist_ok=True)
    ...
    for target, column in [("price", "hpi_benchmark"), ("rent", "rent_avg_city")]:
        ...
        for city, group in df.groupby("city"):
            ...
            # Save model weights
            model_path = f"ml/models/lstm_v2/{city}_{target}.h5"
                ...
                results.append(
                    {
                        ...
                        "features_version": "features_build_etl_v10",
...
if __name__ == "__main__":
    start = datetime.now()
    print("[DEBUG] train_model_lstm_v2 started ...")
    ...
    print(f"\n[DONE] train_model_lstm_v2 completed in {datetime.now() - start}")
