...
def train_lstm_per_city(df: pd.DataFrame):
    ...
    for city, group in df.groupby('city'):
        ...
        model.fit(X_train, y_train, epochs=50, batch_size=8, verbose=0, validation_data=(X_valid, y_valid))
...
def write_predictions(df_preds: pd.DataFrame):
    ...
    print(f"[OK] Inserted {len(df_preds):,} LSTM monthly predictions into public.model_predictions")
