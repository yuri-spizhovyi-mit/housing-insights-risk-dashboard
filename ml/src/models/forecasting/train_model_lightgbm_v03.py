...
----------------------------------------------------------
Unified dual-target LightGBM model training script.
...
# ---------------------------------------------------------------------
# 4. Train LightGBM per city for both targets
# ---------------------------------------------------------------------
def train_lightgbm_dual_target(df: pd.DataFrame):
    results = []
    metrics = []
    ...
    for target, column in [("price", "hpi_benchmark"), ("rent", "rent_avg_city")]:
        ...
        for city, group in df.groupby("city"):
            ...
            if group[column].isna().all() or group[column].nunique() <= 1:
            ...
            valid_mask = (group["date"] > "2020-12-01") & (
            ...
            print(
                f"[OK] LightGBM trained for {city}/{target} ({len(group)} records, 120 forecasts)"
            )
...
def write_predictions(df_preds: pd.DataFrame):
    ...
    print(
        f"[OK] Inserted {len(df_preds):,} LightGBM predictions (both targets) into public.model_predictions"
    )
...
if __name__ == "__main__":
    start = datetime.now()
    print("[DEBUG] train_model_lightgbm_v1 started ...")

    df_features = load_model_features()
    df_preds, df_metrics = train_lightgbm_dual_target(df_features)
    ...
    print(f"\n[DONE] train_model_lightgbm_v1 completed in {datetime.now() - start}")
