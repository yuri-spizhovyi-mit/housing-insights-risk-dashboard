            ...
            try:
                ...
                if not valid_df.empty:
                    ...
                    mae, mape = evaluate_performance(
                        valid_df[column].values, forecast_val.values
                    )
                    metrics.append(
                ...
                print(
                    f"[OK] ARIMA trained for {city}/{target} ({len(group)} records, 120 forecasts)"
                )
...
def write_predictions(df_preds: pd.DataFrame):
    ...
    print(
        f"[OK] Inserted {len(df_preds):,} ARIMA predictions (both targets) into public.model_predictions"
    )
