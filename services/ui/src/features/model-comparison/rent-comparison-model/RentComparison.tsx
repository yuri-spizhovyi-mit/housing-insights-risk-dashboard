...
import RentComparisonChart from "./RentConparisonChart";

function RentComparison() {
  const { city } = useFilters();
  const { modelsData, error, isFetching } = useModelComparison(city, "rent");
    ...
    <Frame className="col-span-12 sm:col-span-8 opacity-100 flex flex-col">
      {isFetching ? (
        ...
        <Frame.Header
          ...
          title="Rent Price Forecast Model Comparison"
        />
      )}
      ...
      <Frame.Body className="min-h-64 flex-1">
        {isFetching ? (
          ...
          <RentComparisonChart modelsData={memoizedData} />
        )}
      </Frame.Body>
    </Frame>
  );
